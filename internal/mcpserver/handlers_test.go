...
func TestToolHandlers(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			wantReqs: []fakeReq{{Method: "POST", Path: "/api/vm/gpio", Body: `"type":"power","duration":800`}},
		...
		},
		{
			...
			serve:    map[string]any{"/api/vm/gpio": map[string]any{}},
			wantReqs: []fakeReq{{Method: "POST", Path: "/api/vm/gpio", Body: `"type":"power","duration":5000`}},
		...
		},
		{
			...
			serve:    map[string]any{"/api/vm/gpio": map[string]any{}},
			wantReqs: []fakeReq{{Method: "POST", Path: "/api/vm/gpio", Body: `"type":"reset","duration":800`}},
		...
		},
		{
			...
			wantReqs: []fakeReq{{Method: "POST", Path: "/api/storage/image/mount", Body: `"file":"/data/debian.iso","cdrom":true`}},
			wantText: "mounted /data/debian.iso",
			audited:  true,
		},
		{
			name:     "unmount_iso posts an empty file",
			tool:     "nanokvm_unmount_iso",
			serve:    map[string]any{"/api/storage/image/mount": map[string]any{}},
			wantReqs: []fakeReq{{Method: "POST", Path: "/api/storage/image/mount", Body: `"file":""`}},
			wantText: "unmounted",
			audited:  true,
		},
		{
